    ...
    class UserRouterClass {
        ...
        routes(){
            ...
            userRouter.get( '/', (req, res) => {
                ...
                // Check fields in the body
                const { ok, extra, miss } = checkFields( [ 'name' ], req.body );

                //=> Error: bad fields provided
                if( !ok ) res.json( { msg: 'Bad fields provided', data: { miss: miss, extra: extra } } )
                else{
                    // Find user new user
                    readItem(req.body, res)
                    .then( apiResponse => res.json( { msg: 'User find', data: apiResponse } ) )
                    .catch( apiResponse => res.json( { msg: 'User not find', data: apiResponse } ) );
                }
            })
